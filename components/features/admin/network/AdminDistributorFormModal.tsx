...
interface AdminDistributorFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (parentMdId: string, data: CreateDistributorInput | UpdateDistributorInput) => Promise<boolean>;
  initialData?: Distributor | null;
  mode: 'create' | 'edit';
}

export const AdminDistributorFormModal: React.FC<AdminDistributorFormModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.trim()) newErrors.name = 'Distributor Full Name is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await onSubmit(parentMdId, payload);
      if (success) {
        onClose();
      }
    } catch (err) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={mode === 'create' ? 'Add New Distributor (Admin Portal)' : `Edit Distributor - ${initialData?.code || ''}`}
      size="xl"
    >
      <form onSubmit={handleSubmit} className="space-y-4 text-slate-800">
        ...
        <div className="bg-indigo-50/50 border border-indigo-100 rounded-lg p-3 space-y-2">
          <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wide flex items-center gap-1.5">
            ...
            Parent Master Distributor Assignment *
          </p>
          <Select
            value={parentMdId}
            ...
            options={masterDistributors.map((md) => ({
              label: `${md.name} (${md.code}) - ${md.businessName}`,
              value: md.id,
            }))}
          />
        </div>
