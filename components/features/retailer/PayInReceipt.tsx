...
import { PayInPreviewResult } from '@/services/payInService';
import { formatCurrency, formatDateTime } from '@/utils/formatters';
import { Button, StatusBadge } from '@/components/ui';
import { Printer, Copy, Check, ShieldCheck, Store, User, ArrowDownLeft } from 'lucide-react';

interface PayInReceiptProps {
...
export const PayInReceipt: React.FC<PayInReceiptProps> = ({
  transaction,
  preview,
  ...
  const [copyType, setCopyType] = useState<'RETAILER' | 'CUSTOMER'>('RETAILER');
  ...
  const handleCopySummary = () => {
    const text = `
=== QIN STAR PAY - PAY-IN RECEIPT ===
Txn ID: ${transaction.transactionRef}
Date: ${formatDateTime(transaction.createdAt)}
Status: ${transaction.status}
Retailer: ${retailerName} (${retailerCode})
Customer Mobile: ${transaction.customerMobile || 'N/A'}
Service: ${transaction.service || 'UPI Pay-In Collection'}
      ...
      <div className="p-6 rounded-2xl border border-slate-300 bg-white shadow-md text-slate-900 space-y-6 print:border-none print:shadow-none print:p-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-100 border text-slate-700">
                {copyType === 'RETAILER' ? 'Retailer Copy' : 'Customer Copy'}
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-0.5">Pay-In Transaction Receipt</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs">
          ...
          <div className="p-3 rounded-lg border border-slate-100 bg-slate-50/50 space-y-1">
            ...
            <p className="font-semibold text-slate-900">{transaction.customerName || 'Walk-in Customer'}</p>
            <p className="text-slate-600 font-mono text-[11px]">Mobile: {transaction.customerMobile || 'N/A'}</p>
            {transaction.orderId && (
              <p className="text-slate-500 font-mono text-[11px]">Ref: {transaction.orderId}</p>
            )}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/70 space-y-2 text-xs font-mono">
          ...
          {transaction.utr && (
            <div className="flex justify-between py-1 border-b border-slate-200">
              <span className="text-slate-500">UTR / RRN Number:</span>
              <span className="font-bold text-emerald-700">{transaction.utr}</span>
            </div>
          )}
          <div className="flex justify-between py-1 border-b border-slate-200">
            <span className="text-slate-500">Service Category:</span>
            <span className="font-semibold text-slate-800">{transaction.service || 'UPI Pay-In Collection'}</span>
          </div>
          <div className="flex justify-between py-1">
            <span className="text-slate-500">Payment Mode:</span>
            <span className="font-semibold text-slate-800">{transaction.paymentMode || 'UPI'}</span>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-slate-200 text-center text-[10px] text-slate-400 space-y-0.5">
          <p className="flex items-center justify-center gap-1 font-semibold text-slate-500">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Authorized Qin Star Pay Merchant Receipt
          </p>
          <p>Thank you for using Qin Star Pay collection switch services.</p>
        </div>
      </div>
    </div>
  );
